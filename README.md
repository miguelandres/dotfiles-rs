...
An action is a task that `dotfiles-rs` can execute. Currently the following
...
Actions are specified in the `steps` object in a YAML configuration file to
define the steps that `dotfiles-rs` executes.
...
* Action types can be invoked more than once to generate the actions in the right
  order
* Some action types (create, link, exec) have short-hand versions and versions
  where more settings can be specified
* Actions support both absolute paths (or relative to the home dir, `~`) and
  paths relative to the configuration file

**Hint**: All actions have a `skip_in_ci` setting, which defaults to false,
if you set it to true the step will be skipped
...
* A subcontext is created by invoking a file from another using the `subconfig`
  action. Defaults are inherited from the parent context (file) and can be
  overridden at any level if necessary
